...
class DeterministicEngine:
    ...
    def extract_candidates(
        ...
        for clause in clauses:
            # 1. Extract Base Medicine Name
            base_name, med_span, med_conf = extract_medicine_candidate(
                clause.text,
                seed_drug_name=clause.seed_drug_name,
                drug_repo=self.drug_repo,
            )

            # If no medicine found in this clause, check if it contains instructions, duration, or frequency to bind
            if not base_name:
                if candidates and (clause.text or clause.advice_clauses):
                    last_cand = candidates[-1]

                    # Trailing duration binding (e.g. "... and also take three times daily for 7 days.")
            ...
            # 2. Extract Strength and Formulation Binding
            full_drug_name, strength, str_span, str_conf = extract_strength_and_formulation(
                clause.text,
            ...
            # 5. Extract Route
            route, route_span, route_conf = extract_route(
                clause.text,
                base_drug_name=base_name,
                drug_repo=self.drug_repo,
            )

            # 6. Extract Primary & Additional Instructions
