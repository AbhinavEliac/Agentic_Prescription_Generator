...
agents/utils.py
---------------
Shared helper utilities, JSON parsers, and multi-medicine clause segmenters.
"""
import re
import json
...
ACTION_VERBS_PATTERN = (
    ...
    r"take\s+tablet\s+of|take\s+tab\s+of|take\s+capsule\s+of|take|"
...
def segment_prescription(input_text: str) -> List[Dict[str, Any]]:
    ...
    """
    clean_text = re.sub(SPEAKER_HEADER_PATTERN, ". Take ", input_text).strip()

    # 1. First break into sentences by standard sentence punctuation (decimal-safe)
    ...
    split_pattern = (
        r"(?i)(?:,\s*(?:and\s+take|also\s+take|additionally(?:,\s*take)?|then\s+take|take\s+one|take\s+two|take\s+10\s*ml|take|"
        ...
        r"\band\s+take\b|\balso\s+take\b|\badditionally\b|\bthen\s+take\b|"
    ...
    NON_DRUG_WITH = (
        ...
        "doctor", "review", "evaluation", "prescription", "consultation", "food", "stomach"
    )
    ...
    for clause in clauses:
        ...
        has_med_indicator = bool(
            re.search(r"(?i)\b(?:take|administer|give|prescribe|start|consume|dissolve|inhale|apply|put|instill|inject|infuse|tablet|tab|capsule|cap|rotacap|vial|sachet|syrup|gel|drops?|spray|ointment|cream|mg|g|mcg|ml|iu)\b", clause_clean)
        )
        ...
        if comp_match:
            ...
            if d2_first_word not in NON_DRUG_WITH and len(d2_first_word) >= 3:
